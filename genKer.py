...
class rhs_info:
	"""
			...
			Args:
				...
				consvar (list): this argument is optional. It specifies which variables to be premultiplied by the density before marching in time. A 'rho' symbol is needed in varname. 
				...
				varloc (dictionary): this argument is optional. It contains names of symbolic expressions used to rename complex algebraic expressions to facilitate equation writing (no intermediate store is performed). Can not contain derivatives yet. 
...
# fltDicBc[1] = [i*0.5 for i in flt3o2]
				...
				if bcname in bc_info: # physical BC name already generated
							...
							if bcdir == cornerperm[bcdir]:
								...
								for k in var2process:
									if var2process[k] != {}:
										...
										def genStored(st,k,indrange,var2process,Stencil,Order,loopst,locst,bcdir,update=update,updateqbc=updateqbc, rhs=rhs):
										...
										genStored(st,k,indrange,var2process,Stencil,Order,loopst,locst,bcdir,update=update,updateqbc=updateqbc, rhs=rhs)	
										# print("Starting P1")
										# sys.stdout.flush()
										# p1 = mp.Process(target=genStored,args=(st,k,indrange,var2process,Stencil,Order,loopst,locst,bcdir,update,updateqbc, rhs,))
										# p1.start()
										# procPipe.append(p1)
					...
					for bcdir in setbc[1][bcname].keys():
						...
						if bcdir == cornerperm[bcdir]:
							...
							for k in var2process:
								if var2process[k] != {}:
									st       = incPATH+'bcsrc'+k+'_'+bcdir+'.for' # set to empty													
									locst    = incPATH+'include_BClocVar_'+k+'_'+bcdir+'.f90'  # set to empty							
									loopst   = incPATH+'include_BCLoops_'+k+'_' +bcdir+'.f90'  # set to empty
									...
									def genStored2(st,k,indrange,var2process,Stencil,Order,loopst,locst,bcdir,update,updateqbc,rhs):

										st       = open(st,'w') # set to empty													
										locst    = open(locst,'w') # set to empty							
										loopst   = open(loopst,'w')  # set to empty
									...
									# genStored(st,k,indrange,var2process,Stencil,Order,loopst,locst,bcdir,update,updateqbc,rhs)	

									p2 = mp.Process(target=genStored2,args=(st,k,indrange,var2process,Stencil,Order,loopst,locst,bcdir,update,updateqbc,rhs,))
									p2.start()
									procPipe.append(p2)
			...
			if dirBC not in edgeBCs:
				...
				if update == False:
					...
					if setbc[0]: 
						...
						localvar = incPATH+'include_PhyBClocVar_'+bcname+'_'+dirBC+'_'+bctype+'.f90'
						output   = incPATH+'include_PhyBCLoops_'+bcname+'_'+dirBC+'_'+bctype+'.f90' 
						rhs_for  = incPATH+'PhyBC'+bcname+'_'+dirBC+'_'+bctype+'.for'

						def create_bcsubroutineMP(rhs_for,bcname,dirBC,bctype,bcnum,indrange,rhs):

							# localvar = open(localvar,'w')
							# output   = open(output ,'w')
							rhs_for  = open(rhs_for,'a+')	
						...
						if len(procPipe) <= mp.cpu_count():
								p = mp.Process(target=create_bcsubroutineMP,args=(rhs_for,bcname,dirBC,bctype,bcnum,indrange,rhs,))
								p.start()
								procPipe.append(p)
						else:
								p0 = procPipe.pop(0)
						...
						for layer in range(0,hlo_rhs): #BC layers  
							...
							localvar = incPATH+'include_BClocVar_'+dirBC+'_'+str(layer)+'.f90'
							output   = incPATH+'include_BCLoops_'+dirBC+'_'+str(layer)+'.f90' 	
							rhs_for  = incPATH+'bcsrc'+dirBC+'_'+str(layer)+'.for'

							outlayer.append(output)
							locallayer.append(localvar)

							def create_bcsubroutineMP(rhs_for,dirBC,layer,indrange,rhs):
								rhs_for  = open(rhs_for,'a+')
							...
							if len(procPipe) <= mp.cpu_count():
								p = mp.Process(target=create_bcsubroutineMP,args=(rhs_for,dirBC,layer,indrange,rhs,))
								p.start()
								procPipe.append(p)
							else:
								p0 = procPipe.pop(0)
				...
				if setbc[0]:
					...
					def gen_eqns_bcMP(Eqns,output,localvar,rhsname,Order,Stencil,indi,indj,indk,DirDic,vname,update,updateq,rhs):
					...
					if len(procPipe) <= mp.cpu_count():
								p = mp.Process(target=gen_eqns_bcMP,args=(Eqns,output,localvar,rhsname,Order,Stencil,indi,indj,indk,DirDic,vname,update,updateq,rhs,))
								p.start()
								procPipe.append(p)
					else:
								p0 = procPipe.pop(0)
					...
					for layer in range(0,hlo_rhs): #BC layers 
						...
						def gen_eqns_bcMP(Eqns,output,localvar,rhsname,Order,Stencil,indi,indj,indk,DirDic,vname,update,rhs):
						...
						if len(procPipe) <= mp.cpu_count():
								p = mp.Process(target=gen_eqns_bcMP,args=(Eqns,output,localvar,rhsname,Order,Stencil,indi,indj,indk,DirDic,vname,update,rhs,))
								p.start()
								procPipe.append(p)
						else:
								p0 = procPipe.pop(0)
...
#											
											...
											localvar = incPATH+'include_edges_'+dir1+'_'+dir2+'_'+str(layer1)+'_'+str(layer2)+'_BClocVar.f90'
											output   = incPATH+'include_edges_'+dir1+'_'+dir2+'_'+str(layer1)+'_'+str(layer2)+'_BCLoops.f90' 
											...
											if stored:	
												...
												for k in var2process:
													if var2process[k] != {}:
														...
														localvar_stored[k] = incPATH+'include_'+k+'_edges_'+dir1+'_'+dir2+'_'+str(layer1)+'_'+str(layer2)+'_BClocVar.f90'
														output_stored[k]   = incPATH+'include_'+k+'_edges_'+dir1+'_'+dir2+'_'+str(layer1)+'_'+str(layer2)+'_BCLoops.f90' 
											...
											def gen_eqns_bcMP(Eqns,localvar,output,rhsname,Order,Stencil,indi,indj,indk,DirDic,vname,update,rhs):	
											...
											if len(procPipe) <= mp.cpu_count():
													p = mp.Process(target=gen_eqns_bcMP,args=(Eqns,localvar,output,rhsname,Order,Stencil,indi,indj,indk,DirDic,vname,update,rhs,))
													p.start()
													procPipe.append(p)
											else:
													p0 = procPipe.pop(0)
											...
											if stored:
												...
												def gen_eqnsStored_bcMP(var2process,output_stored,localvar_stored,Order,Stencil,indi,indj,indk,DirDic,rhs):
													...
													for k in var2process:
														if var2process[k] != {}:
															...
															output_IO   = open(output_stored[k],'a+')	
															localvar_IO = open(localvar_stored[k],'a+')	
		
															gen_eqns_bc(var2process[k],output_IO,localvar_IO,
												...
												if len(procPipe) <= mp.cpu_count():
														p = mp.Process(target=gen_eqnsStored_bcMP,args=(var2process,output_stored,localvar_stored,Order,Stencil,indi,indj,indk,DirDic,rhs,))
														p.start()
														procPipe.append(p)
												else:
														p0 = procPipe.pop(0)
...
#
											...
											localvar    = incPATH+'include_edges_PhyBC_'+bcname+'_'+bctype+'_'+dir1+'_'+dir2+'_0'+'_'+str(layer2)+'_BClocVar.f90'
											outputPhy   = incPATH+'include_edges_PhyBC_'+bcname+'_'+bctype+'_'+dir1+'_'+dir2+'_0'+'_'+str(layer2)+'_BCLoops.f90' 
											...
											def gen_eqns_bcPHYMP(Eqns,outputPhy,localvar,rhsname,Order,Stencil,indi,indj,indk,DirDic,vname,update,updateq,rhs):	

												localvar    = open(localvar ,'a+')
												outputPhy   = open(outputPhy,'a+')
											...
											if len(procPipe) <= mp.cpu_count():
													p = mp.Process(target=gen_eqns_bcPHYMP,args=(Eqns,outputPhy,localvar,rhsname,Order,Stencil,indi,indj,indk,DirDic,vname,update,updateq,rhs,))
													p.start()
													procPipe.append(p)
											else:
													p0 = procPipe.pop(0)
...
#			 
					...
					localvar    = incPATH+'include_edges_PhyBC_'+bcname+'_'+bctype+'_'+dir1+'_0_0'+'_BClocVar.f90'
					outputPhy   = incPATH+'include_edges_PhyBC_'+bcname+'_'+bctype+'_'+dir1+'_0_0'+'_BCLoops.f90' 
					...
					if dir2[0] != dir1[0]:
						...
						if ((dir2+dir1 not in edone) and (dir1+dir2 not in edone)):
							if ((dir1+dir2 not in bcdone) and (dir2+dir1 not in bcdone)):
								...
								if (dir1+dir2) in bcphy_all or (dir2+dir1) in bcphy_all:
									
									if (dir1+dir2) in bcphy_all:
											dir1dir2 = dir1+dir2
									else:
											dir1dir2 = dir2+dir1
